...
class Cell
  ...
  attr_accessor :sky, :dug, :rock, :oom, :x, :y, :rect #this is kind of ridiculous, i probably shouldn't expose so many attributes
  ...
  def initialize(coords, options = {})
    ...
    @x, @y = @coords[0], @coords[1]
		...
		options[:type] == :oom ? @oom = true : @oom = false
  ...
  def draw(screen, options = {})
    ...
    outline
    ladderize if @ladder
		xi, yi = options[:x], options[:y]
		if xi && yi
			set_center(:x => xi, :y => yi)
		end
    super(screen)
  end
